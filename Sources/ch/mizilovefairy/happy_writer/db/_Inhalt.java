...
public abstract class _Inhalt extends ERXGenericRecord {
	...
	public static final ERXKey<ch.mizilovefairy.happy_writer.db.Artikel> ARTIKEL = new ERXKey<ch.mizilovefairy.happy_writer.db.Artikel>(
			"artikel", Type.ToManyRelationship);
	...
	public NSArray<ch.mizilovefairy.happy_writer.db.Artikel> artikel(EOQualifier qualifier,
		...
		results = artikel();
		if (qualifier != null) {
			results = (NSArray<ch.mizilovefairy.happy_writer.db.Artikel>) EOQualifier
					.filteredArrayWithQualifier(results, qualifier);
		}
		if (sortOrderings != null) {
			results = (NSArray<ch.mizilovefairy.happy_writer.db.Artikel>) EOSortOrdering
	...
	public ch.mizilovefairy.happy_writer.db.Artikel createArtikelRelationship() {
		EOEnterpriseObject eo = EOUtilities.createAndInsertInstance(editingContext(),
				ch.mizilovefairy.happy_writer.db.Artikel.ENTITY_NAME);
	...
	public NSArray<ch.mizilovefairy.happy_writer.db.BestellPosition> bestellPositionen() {
		return (NSArray<ch.mizilovefairy.happy_writer.db.BestellPosition>) storedValueForKey(
				_Inhalt.BESTELL_POSITIONEN_KEY);
	}
