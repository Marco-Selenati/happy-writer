...
public abstract class _Kunde extends  ERXGenericRecord {
  ...
  // Relationship Keys
  public static final ERXKey<ch.mizilovefairy.happy_writer.db.Bestellung> BESTELLUNGEN = new ERXKey<ch.mizilovefairy.happy_writer.db.Bestellung>("bestellungen", Type.ToManyRelationship);
  ...
  public NSArray<ch.mizilovefairy.happy_writer.db.Bestellung> bestellungen(EOQualifier qualifier, NSArray<EOSortOrdering> sortOrderings, boolean fetch) {
    ...
    if (fetch) {
      ...
      results = ch.mizilovefairy.happy_writer.db.Bestellung.fetchBestellungs(editingContext(), fullQualifier, sortOrderings);
    }
    else {
      results = bestellungen();
      if (qualifier != null) {
        results = (NSArray<ch.mizilovefairy.happy_writer.db.Bestellung>)EOQualifier.filteredArrayWithQualifier(results, qualifier);
      }
      if (sortOrderings != null) {
        results = (NSArray<ch.mizilovefairy.happy_writer.db.Bestellung>)EOSortOrdering.sortedArrayUsingKeyOrderArray(results, sortOrderings);
      }
    }
    return results;
  }
  ...
  public ch.mizilovefairy.happy_writer.db.Bestellung createBestellungenRelationship() {
    EOEnterpriseObject eo = EOUtilities.createAndInsertInstance(editingContext(),  ch.mizilovefairy.happy_writer.db.Bestellung.ENTITY_NAME );
  ...
  public void deleteAllBestellungenRelationships() {
    Enumeration<ch.mizilovefairy.happy_writer.db.Bestellung> objects = bestellungen().immutableClone().objectEnumerator();
